...
const mdComponents: Components = {
  ...
  h1: ({ children }) => (
    <h1 className="font-['Newsreader'] text-2xl sm:text-3xl font-bold text-[#1a1b1c] mt-6 mb-3 leading-tight break-words">{children}</h1>
  ),
  h2: ({ children }) => (
    <h2 className="font-['Newsreader'] text-xl sm:text-2xl font-semibold text-[#1a1b1c] mt-5 mb-2 leading-snug break-words">{children}</h2>
  ),
  h3: ({ children }) => (
    <h3 className="font-['Newsreader'] text-lg sm:text-xl font-semibold text-[#313334] mt-4 mb-1 break-words">{children}</h3>
  ),
  p: ({ children }) => (
    <p className="text-[#313334] text-base sm:text-[1.05rem] leading-[1.8] my-2 break-words">{children}</p>
  ),
  ul: ({ children }) => (
    <ul className="list-disc pl-5 sm:pl-6 my-3 flex flex-col gap-1.5 text-base sm:text-[1.05rem] text-[#313334]">{children}</ul>
  ),
  ol: ({ children }) => (
    <ol className="list-decimal pl-5 sm:pl-6 my-3 flex flex-col gap-1.5 text-base sm:text-[1.05rem] text-[#313334]">{children}</ol>
  ),
  li: ({ children }) => <li className="leading-relaxed break-words">{children}</li>,
  blockquote: ({ children }) => (
    <blockquote className="border-l-4 border-[#585f64] pl-4 sm:pl-5 my-4 text-[#5e5f61] italic text-base sm:text-lg font-['Newsreader']">{children}</blockquote>
  ),
  table: ({ children }) => (
    <div className="overflow-x-auto my-4 rounded-xl border border-[#e4e2e1] max-w-full">
  ...
  a: ({ href, children }) => (
    <a href={href} target="_blank" rel="noopener noreferrer"
      className="text-[#585f64] underline underline-offset-2 hover:text-[#313334] transition-colors break-all">
      {children}
    </a>
  ),
}
...
const EditorPage = ({ searchParams }: { searchParams: Promise < { id ? : string } > }) => {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/articles' + id ? "/"+id : null, {
        ...
        return (
          ...
          <div className="flex items-center gap-3 p-4 bg-white rounded-xl border border-[#e4e2e1]">
            <div className="relative w-12 h-12 shrink-0">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                ...
                <circle cx="18" cy="18" r="15.9" fill="none"
                  ...
                  strokeWidth="3" strokeDasharray={`${seoScore} 100`} strokeLinecap="round" />
              </svg>
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-[#5e5f61] break-words">
      ...
      case 'accessibility':
        return (
          ...
          <div className="p-4 bg-[#fef9c3] rounded-xl border border-[#fde68a]">
            ...
            <p className="text-[11px] text-[#78350f] mt-1 leading-relaxed break-words">
              Add alt text to images: <code className="bg-[#fde68a]/50 px-1 rounded text-[10px] break-all">![alt text](url)</code>
            </p>
          </div>
        </div>
        )
  ...
  const SidebarInner = () => (
    <div className="flex flex-col h-full min-w-0 overflow-hidden">
      <div className="px-4 pb-3 shrink-0">
        ...
        <p className="text-[10px] text-[#9e9fa0] mt-0.5">Article ID: 8821</p>
      </div>
      <nav className="flex flex-col gap-0.5 px-1 shrink-0">
        ...
        ] as { id: SidebarTab; icon: React.ReactNode; label: string }[]).map(({ id, icon, label }) => (
          <SidebarLink key={id} icon={icon} label={label} active={activeTab === id}
    ...
    <div className="min-h-screen bg-white text-black">
      ...
      <header className="w-full sticky top-0 z-50 bg-[#fcf8f9]/95 backdrop-blur-md border-b border-[#e4e2e1]">
        <div className="flex items-center justify-between px-3 sm:px-5 lg:px-8 py-2.5 gap-2 min-w-0">
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            {/* Undo/redo — sm+ only (also in toolbar on mobile) */}
            <button onClick={undo} disabled={historyIndex <= 0} title="Undo"
              className="hidden sm:block p-1.5 text-[#585f64] hover:bg-black/5 rounded-lg disabled:opacity-30 shrink-0">
              <RotateCcw size={15} />
            </button>
            ...
            {/* History — lg+ */}
            <button title="View history" className="hidden lg:block p-1.5 text-[#585f64] hover:bg-black/5 rounded-lg shrink-0">
              <History size={15} />
            </button>

            {/* Desktop sidebar toggle — xl+ */}
            <button title="Toggle sidebar" onClick={() => setSidebarOpen(!sidebarOpen)}
              className="hidden xl:block p-1.5 text-[#585f64] hover:bg-black/5 rounded-lg shrink-0">
              <PanelLeft size={15} />
            </button>

            {/* Mobile settings — below xl */}
            <button title="Article settings" onClick={() => setMobileDrawerOpen(true)}
              className="xl:hidden p-1.5 text-[#585f64] hover:bg-black/5 rounded-lg shrink-0">
      ...
      <div className="flex overflow-hidden" style={{ height: 'calc(100dvh - 113px)' }}>
        ...
        <main className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          <div className="flex items-center px-2 sm:px-3 py-1.5 bg-white overflow-x-auto shrink-0"
            ...
            {/* Undo/redo in toolbar on mobile */}
            ...
            <span className="hidden sm:block text-[10px] text-[#9e9fa0] whitespace-nowrap pr-1 shrink-0">
              {countWords(content)}w · ~{estimateReadTime(content)}min
            </span>
          </div>
                ...
                <div className="mx-auto px-4 sm:px-8 py-8 sm:py-12 w-full max-w-3xl min-w-0 overflow-hidden">
                  ...
                  {coverImage && (
                    <figure className="mb-6 rounded-xl overflow-hidden">
                      <img src={coverImage} alt="Cover" className="w-full object-cover max-h-48 sm:max-h-72" />
                    </figure>
                  )}

                  {title && (
                    <h1 className="font-['Newsreader'] text-3xl sm:text-4xl lg:text-5xl font-bold text-[#1a1b1c] leading-tight mb-4 break-words">
                      {title}
                    </h1>
                  )}
      ...
      {mobileDrawerOpen && (
        <>
          ...
          <div className="fixed top-0 right-0 bottom-0 z-[70] bg-[#f5f3f3] flex flex-col shadow-2xl xl:hidden overflow-hidden"
      ...
      {/* ── Publish modal — bottom sheet on mobile, centered on sm+ ────────── */}
      {showPublishModal && (
