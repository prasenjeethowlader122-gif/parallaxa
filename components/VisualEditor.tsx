...
function URLModal({ title, placeholder, label, onConfirm, onClose }: {
  ...
  return (
    ...
    >
      <div
          className="bg-white rounded-xl p-5 w-full max-w-sm mx-4"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-sm text-gray-900">{title}</h3>
          <button onClick={onClose} className="p-1 text-gray-400 hover:text-gray-600 rounded-xl transition-colors">
            <X size={14} />
          </button>
        </div>
        <input
...
const ToolBtn = ({ onClick, active, disabled, title, children }: {
  ...
  <button
    ...
    disabled={disabled}
    title={title}
    className={`w-9 h-9 rounded-xl transition-all shrink-0 flex items-center justify-center ${
      active
  ...
  return (
    <div className="w-full flex flex-col relative">
      ...
      >
        <div className="flex items-center gap-0.5 bg-gray-900 rounded-xl p-1">
          ...
          <button
            ...
            className={`w-9 h-9 rounded-xl text-[10px] font-bold transition-all ${editor.isActive('heading', { level: 1 }) ? 'bg-white text-gray-900' : 'text-white/80 hover:bg-white/10 hover:text-white'}`}
            title="H1"
          >H1</button>
          <button
            onMouseDown={e => { e.preventDefault(); editor.chain().focus().toggleHeading({ level: 2 }).run() }}
            className={`w-9 h-9 rounded-xl text-[10px] font-bold transition-all ${editor.isActive('heading', { level: 2 }) ? 'bg-white text-gray-900' : 'text-white/80 hover:bg-white/10 hover:text-white'}`}
            title="H2"
          >H2</button>
          <BubbleSep />
      ...
      {slashMenu && (
        <div
          ref={slashRef}
          className="absolute z-30 bg-white border border-gray-100 rounded-xl py-2 w-56 animate-in fade-in zoom-in-95 duration-100"
        ...
        >
          ...
          {SLASH_ITEMS.map((item) => (
            ...
            >
              <span className="w-8 h-8 flex items-center justify-center rounded-xl bg-gray-100 text-gray-500 shrink-0">
                {item.icon}
              </span>
              {item.label}
            </button>
